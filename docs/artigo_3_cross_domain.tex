...
On seismic data, the best model (Tiny TCN, 5{,}273 parameters)
achieves AUC-PR~$=0.9144$, VUS-PR~$=0.8921$, PA-F1~$=0.9443$, and
FP/h~$=3.144$.
Evaluation on the 3W domain is prepared (adapter, profile, and benchmark
script are implemented and validated) and will be completed upon public
download of the Petrobras 3W dataset (available at
\url{https://github.com/petrobras/3W}).
The architecture, contract definition, and seismic benchmarks are
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcc}
    ...
    Anomaly ratio    & $\approx$12.8\% & domain-dependent\dag \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table*}[htbp]
  ...
  \end{tabular}
  \smallskip\par
  \footnotesize \dag~3W results pending dataset download from
  \url{https://github.com/petrobras/3W}. Adapter, profile, and benchmark
  script are implemented and validated; experiment will run upon data availability.
\end{table*}
...
Tiny TCN $>$ Tiny CNN $>$ RF $>$ ET $>$ LR.
$\rho_s$ across domains will be computed upon completion of the 3W experiments
(target: $\rho_s > 0.60$, i.e., Tiny TCN and Tiny CNN remain the top-ranked
models also on 3W).
...
\begin{itemize}
  ...
  \item \textbf{Imbalance:} both are imbalanced but at different ratios
        ($\approx$12.8\% vs.\ domain-dependent in 3W\dag).
\end{itemize}
...
Seismic results are complete: Tiny TCN achieves AUC-PR~$=0.9144$,
VUS-PR~$=0.8921$, PA-F1~$=0.9443$, and FP/h~$=3.144$ with only 5{,}273
parameters, and passes the full quality gate (27.1~KB int8 TFLite export).
The 3W domain arm of the benchmark is ready to run upon dataset download.

...
Without the gate, the worst-performing model (Logistic Regression) produces
FP/h~$=6.568$---$2.1\times$ the gate-approved Tiny TCN baseline of 3.144~FP/h---at
2.09$\times$ the operational cost while being physically undeployable without
the embedded resource gate (Random Forest: 316~MB; Extra Trees: 921~MB).
...
rate, VUS-PR, PA-F1, over-the-air update, ESP32, cross-domain generalization.
\end{IEEEkeywords}
...
see companion paper~\cite{artigo3}), AUC-PR values are not directly
comparable due to varying overlap fractions.
...
select operationally reliable models: without the full gate, the worst
screening-only candidate (Logistic Regression) generates FP/h~$=6.568$,
$2.1\times$ the gate-approved Tiny TCN baseline; ensemble models (RF, ET)
pass AUC-PR but are physically undeployable at 316--921~MB.
...
(ii) extension to multivariate drift detection for multi-sensor domains
    (Petrobras 3W, SKAB, ECG5000, financial stress)---companion paper~3
    documents cross-domain evaluation on five domains revealing distinct
...
\bibitem{artigo3}
[AUTHOR],
``Cross-domain generalization of a reproducible TinyML anomaly detection pipeline,''
[VENUE], 2026. \textit{(companion paper 3)}

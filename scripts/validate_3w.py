"""
Validação cross-domain: modelos treinados no dataset 3W (multivariate, 7 canais).
Artigo 3 — compara Tiny TCN/CNN multivariate vs RF com features cross-canal.
"""
import sys, os
...
print("Carregando dataset 3W (multivariate, 7 canais)...", flush=True)
t0 = time.time()
...
print(f"  {time.time()-t0:.1f}s | train={X_train.shape} | anomaly_test={y_test.mean():.1%}", flush=True)
# X shape: (N, 60, 7) — multivariate: 7 canais de processo @ 1 Hz
...
# ── RF com features cross-canal (28*7 + C(7,2) = 217 features) ───────────────
print("\nRF com features estatísticas + cross-canal...", flush=True)
from src.features.statistical_features import extract_statistical_features

Xtr_s, ytr_s = subsample(X_train, y_train, 200_000)
Xv_s,  yv_s  = subsample(X_val,   y_val,   50_000)

print("  Extraindo features treino...", flush=True)
F_train = extract_statistical_features(Xtr_s, sample_rate=1.0)
print("  Extraindo features val...",   flush=True)
F_val_f = extract_statistical_features(Xv_s,  sample_rate=1.0)
print("  Extraindo features test...",  flush=True)
F_test_f = extract_statistical_features(X_test, sample_rate=1.0)
print(f"  Features: {F_train.shape[1]} por janela", flush=True)

from sklearn.ensemble import RandomForestClassifier
rf = RandomForestClassifier(n_estimators=100, n_jobs=-1, random_state=42, max_depth=15)
rf.fit(F_train, ytr_s)
results["random_forest_3w"] = evaluate(
    "RF-3W (cross-canal)",
    rf.predict_proba(F_val_f)[:,1], rf.predict_proba(F_test_f)[:,1],
    yv_s, y_test,
    n_params=int(rf.n_estimators * rf.estimators_[0].tree_.node_count)
)

# ── Tiny CNN e Tiny TCN multivariate (N, 60, 7) ───────────────────────────────
...
for mname, builder, hparams in model_cfgs:
    print(f"\n{mname}  input=(N, 60, 7) multivariate...", flush=True)
...
out = Path("results/experiment_3w_validation.json")
out.parent.mkdir(exist_ok=True)
with open(out, "w") as f:
    json.dump(results, f, indent=2)

print(f"\nResultados 3W salvos em {out}")
